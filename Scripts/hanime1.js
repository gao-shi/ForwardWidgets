    ...
    modules: [
        {
            ...
            params: [
                ...
                {
                    name: "mode", title: "搜索模式", type: "enumeration", description: "选择检索方式", value: "normal",
                    enumOptions: [
        ...
        },
        {
            ...
            params: [
                {
                    name: "genre", title: "基础大分类", type: "enumeration", description: "选择主分类", value: "all",
                    enumOptions: [
                        { title: "全部", value: "all" }, { title: "里番", value: "rifan" }, { title: "泡面番", value: "paomian" },
                        { title: "Motion Anime", value: "motion" }, { title: "3DCG", value: "3dcg" }, { title: "2D 动画", value: "2d" },
                        { title: "2.5D", value: "2_5d" }, { title: "AI 生成", value: "ai" }, { title: "MMD", value: "mmd" }, { title: "Cosplay", value: "cosplay" }
                    ]
                },
                {
                    name: "sort_by", title: "排序方式", type: "enumeration", description: "排序规则", value: "all",
                    enumOptions: [
                        { title: "全部", value: "all" }, { title: "最新上市", value: "new_release" }, { title: "最新上传", value: "latest_upload" },
                        { title: "本日排行", value: "daily_rank" }, { title: "本周排行", value: "weekly_rank" }, { title: "本月排行", value: "monthly_rank" },
                        { title: "观看次数", value: "views" }, { title: "点赞比例", value: "likes" }, { title: "时长最长", value: "duration" }, { title: "他们在看", value: "watching" }
                    ]
                },
                {
                    name: "upload_time", title: "发布日期", type: "enumeration", description: "筛选发布时间", value: "all",
                    enumOptions: [
                        { title: "全部", value: "all" }, { title: "过去 24 小时", value: "24h" }, { title: "过去 2 天", value: "2d" },
                        { title: "过去 1 周", value: "1w" }, { title: "过去 1 个月", value: "1m" }, { title: "过去 3 个月", value: "3m" }, { title: "过去 1 年", value: "1y" }
                    ]
                },
                {
                    name: "duration_filter", title: "时长范围", type: "enumeration", description: "筛选影片时长", value: "all",
                    enumOptions: [
                        { title: "全部", value: "all" }, { title: "1 分钟 +", value: "1m_plus" }, { title: "5 分钟 +", value: "5m_plus" },
                        { title: "10 分钟 +", value: "10m_plus" }, { title: "20 分钟 +", value: "20m_plus" }, { title: "30 分钟 +", value: "30m_plus" },
                        { title: "60 分钟 +", value: "60m_plus" }, { title: "0 - 10 分钟", value: "0_10m" }, { title: "0 - 20 分钟", value: "0_20m" }
                    ]
                },
                { name: "page", title: "页码", type: "page", description: "页码", value: "1" }
            ]
        },
        {
            ...
            params: [
                {
                    name: "list_type", title: "榜单类型", type: "enumeration", description: "选择你想看的榜单", value: "watching",
                    enumOptions: [
                        { title: "他们在看", value: "watching" }, { title: "最新上传", value: "latest_upload" }, { title: "本日排行", value: "daily_rank" },
                        { title: "本周排行", value: "weekly_rank" }, { title: "本月排行", value: "monthly_rank" }, { title: "最新上市", value: "new_release" }
                    ]
                },
                { name: "page", title: "页码", type: "page", description: "页码", value: "1" }
            ]
            ...
            params: [
                {
                    ...
                    enumOptions: [
                        { title: "全部", value: "all" }, { title: "里番", value: "rifan" }, { title: "泡面番", value: "paomian" },
                        { title: "Motion Anime", value: "motion" }, { title: "3DCG", value: "3dcg" }, { title: "2D 动画", value: "2d" }, { title: "Cosplay", value: "cosplay" }
                    ]
                },
                {
                    name: "sort_by", title: "排序", type: "enumeration", description: "排序方式", value: "all",
...
function getCommonHeaders(refererUrl = BASE_URL + "/") {
    return {
        ...
        "Accept-Language": "zh-CN,zh-Hans;q=0.9,zh-Hant;q=0.8,en;q=0.7",
...
function convertToTraditional(text) {
    if (!text) return "";
    const map = {
        "无码": "無碼", "AI解码": "AI解碼", "断面图": "斷面圖", "里番": "裏番", "泡面番": "泡麵番", 
        "中文字幕": "中文字幕", "中文配音": "中文配音", "傲娇": "傲嬌", "病娇": "病嬌", 
        "性奴隶": "性奴隸", "出轨": "出軌", "性转换": "性轉換", "婚纱": "婚紗", 
        "淫纹": "淫紋", "身体写字": "身體寫字", "舔脚": "舔腳", "一字马": "一字馬"
    };
    let result = text;
...
function mapSortToApi(v) {
    const m = {
        "new_release": "最新上市", "latest_upload": "最新上傳", "daily_rank": "本日排行",
        "weekly_rank": "本週排行", "monthly_rank": "本月排行", "views": "觀看次數",
        "likes": "點讚比例", "duration": "時長最長", "watching": "他們在看"
    };
    return m[v] || "";
}
...
function mapGenreToApi(v) {
    const m = {
        "rifan": "裏番", "paomian": "泡麵番", "motion": "Motion Anime", "3dcg": "3DCG",
        "2d": "2D動畫", "2_5d": "2.5D", "ai": "AI生成", "mmd": "MMD", "cosplay": "Cosplay"
    };
    return m[v] || "";
}
...
async function fetchAndParse(url, referer) {
    try {
        ...
        return [{ id: "error", type: "text", title: "请求失败，请尝试在浏览器中完成Cloudflare验证。" }];
    }
}
...
async function loadHotRankings(params) {
    const page = parseInt(params.page) || 1;
    const sortVal = {
        watching: '他們在看', latest_upload: '最新上傳', daily_rank: '本日排行',
        weekly_rank: '本週排行', monthly_rank: '本月排行', new_release: '最新上市'
...
function parsePreviewsHtml(html, pageUrl) {
    ...
    const selectors = [
        '.preview-image-modal-trigger', '.preview-item img', '.preview-item a',
        '[class*="preview"] img', '[class*="preview"] a', 'a[href*="/previews/"]', 'a[href*="/watch?v="]'
    ];
    ...
    $(selectors.join(',')).each((i, el) => {
        ...
        const rawTitle = stripHtmlText(
            $anchor.attr('title') || $node.attr('alt') || $anchor.find('img').attr('alt') ||
            $node.closest('a').find('img').attr('alt') || $node.closest('div').find('h5.caption, .caption, .title, h5, h4').first().text() ||
            $anchor.text() || $node.text() || ''
        );
        const title = rawTitle.replace(/^(thumb_up|play_arrow|info)\s*/i, '').replace(/^(?:\d{1,2}:\d{2}(?::\d{2})?)\s*/g, '').trim() || '新番预告';

        if (!link || seen.has(link)) return;
        if (!poster && title === '新番预告') return;
        seen.add(link);
...
async function loadPreviews() {
    ...
    const urls = [
        `${BASE_URL}/previews/${ym}`, `${BASE_URL}/previews/${year}-${month}`,
        `${BASE_URL}/previews/${year}${month}`, `${BASE_URL}/previews`, BASE_URL + "/"
    ];
...
async function loadDetail(link) {
    try {
        ...
        for (const candidate of candidates) {
            if (candidate && (candidate.includes('m3u8') || candidate.includes('mp4'))) { 
                videoUrl = candidate; 
                break; 
            }
        }

        // 2. 正则回退：匹配 HTML 中的 m3u8
        if (!videoUrl) {
            const m3u8Match = html.match(/https?:\/\/[^"'\s<>\[\]]+\.m3u8[^"'\s<>\[\]]*/);
            if (m3u8Match) videoUrl = m3u8Match[0];
        }

        // 3. 应对深层 JS 变量中转义的斜杠网址 (https:\/\/...)
        if (!videoUrl) {
            const escapedMatch = html.match(/(https?:\\\/\\\/[^"'\s<>\[\]]+\.m3u8[^"'\s<>\[\]]*)/);
            if (escapedMatch) videoUrl = escapedMatch[1].replace(/\\\//g, '/');
        }

        // 4. 正则回退：匹配 HTML 中的直连 mp4
        if (!videoUrl) {
        ...
        const title = safeText(convertToTraditional($('meta[property="og:title"]').attr('content') || $('title').text() || '未知标题'), '未知标题');
        ...
        let errorMsg = '无法加载视频，可能网站启用了安全防护拦截或该视频已被下架。';
        if (error.message === 'video_url_not_found') errorMsg = '解析失败：未找到视频源，网站结构可能已变更。';
        return {
            id: link,
            type: 'detail',
            videoUrl: link,
